...
function new_word(form)
{
  if(!running)
  {
    ...
    alpha[1]=words[index].charAt(wo<rds[index].length-1);
    alpha_index=1;
...
function getImpiccato() {
  ...
  request.onload = () => {
    ...
    if (request.status == 200) {
      var impiccatoJSON = JSON.parse(request.response)
      console.log(impiccatoJSON);
      impiccatoShow(impiccatoJSON);
    } else {
      console.log(`error ${request.status} ${request.statusText}`)
    }
  }
}
function impiccatoShow(impiccatoJSON) {
    var nome = ["name"]
    console.log(nome);
    }
